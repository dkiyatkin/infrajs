...
У новых слоев необязательно указывать `layer.tag`, так как этот параметр уже задан в его ключе.
...
У новых слоев необязательно указывать `layer.state`, так как этот параметр уже задан в его ключе.
Все новые слои попадают в массив `layer.childs` родительского слоя.
У каждого нового слоя параметр `layer.parent` ссылается на родителя.
